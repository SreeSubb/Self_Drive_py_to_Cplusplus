...
vector< vector<float> > normalize(vector< vector <float> > grid) {
    ...
    for(int i = 0; i < grid.size(); i++) {
        for(int j = 0; j < grid[0].size(); j++) {
            newGrid.emplace_back(grid[i][j] / sum);
        }
    }
	return newGrid;
}
        ...
        for(int j = 0; j < height; j++) {
			for(int dx = -1; dx < 2; dx++) {
				for(int dy = -1; dy < 2; dy++) {
					float blurrFactor = window[dx+1][dy+1];
					int new_i = (((i + dy) % height) + height) % height;
					int new_j = (((j + dx) % width) + width) % width;
					//int new_i = (i + dx) % grid.size();
					//int new_j = (j + dy) % grid[0].size();
					newGrid[new_i][new_j] += blurrFactor * grid[i][j];
				}
			}
        }
    }
